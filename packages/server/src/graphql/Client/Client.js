...
export const typeDefs = gql`
  ...
  extend type Mutation {
    ...
    updateClient(input: UpdateClient!): Client!
...
export const resolvers = {
  Query: {
    ...
    clients: async (_, args) => {
      const { skip = 0, take, sort, filter } = args.options || {};
  ...
  Mutation: {
    ...
    updateClient: async (_, { input }) => {
      ...
      const updatedClients = clients.map((_client) => {
        if(_client.id === updatedClient.id) return updatedClient;
